...
"""
RazorPay Charger API - النسخة الكاملة المتكاملة
"""
...
# ============================================================
# PROXY MANAGER - مأخوذ من الأداة الأولى
# ============================================================
...
# ============================================================
# FINGERPRINT GENERATOR - مأخوذ من الأداة الأولى
# ============================================================
...
def charge_batch(cards, site_url, amount=5, currency='USD', max_workers=5, proxy_manager=None):
    """شحن مجموعة بطاقات - مأخوذ من الأداة الأولى"""
    results = []
    success_count = 0
    fail_count = 0
    completed = 0
...
def razorpay_checker():
    """شحن بطاقة واحدة - Endpoint أساسي"""
    try:
...
@app.route('/razorpay_batch', methods=['POST'])
def razorpay_batch():
    """شحن مجموعة بطاقات - Endpoint جديد"""
    try:
        ...
        amount = data.get('amount', 'random')
        currency = data.get('currency', 'USD')
        threads = min(data.get('threads', 5), 20)
        ...
        if proxies:
            ...
            if isinstance(proxies, list):
                ...
                proxy_manager.load_from_string(proxies)
        
        # Process batch
        start_time = time.time()
        result = charge_batch(cards, site_url, amount, currency, threads, proxy_manager)
...
@app.route('/razorpay_stats', methods=['GET'])
def razorpay_stats():
    """إحصائيات الـ API"""
    return jsonify({
